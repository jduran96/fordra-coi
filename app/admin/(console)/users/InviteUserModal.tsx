...
/**
 * "Invite User" button + modal: email an invite and assign the org in one
 * step. The dialog is remounted (key) on every open so a previous invite's
...
export default function InviteUserModal({ orgs }: { orgs: { id: string; name: string }[] }) {
  ...
  return (
    <>
      <button onClick={() => { setSession(s => s + 1); setOpen(true) }} style={primaryBtn}>Invite User</button>
      {open && <InviteDialog key={session} orgs={orgs} onClose={() => setOpen(false)} />}
    </>
  )
}

function InviteDialog({ orgs, onClose }: { orgs: { id: string; name: string }[]; onClose: () => void }) {
            ...
            <form action={formAction} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <label style={lbl}>Org
                <select name="org_id" required defaultValue="" style={input}>
                  ...
                  <option value="none">No organization (assign later)</option>
                </select>
              </label>
