...
export const CONFIG_KEYS = {
  ...
  promptInsurerQuestions: 'prompt_insurer_questions',   // string — system prompt for the insurer call questions
} as const
...
export async function getExtractionConfig(): Promise<ExtractionConfig> {
  ...
  return {
    ...
    promptInsurerQuestions: (map.get(CONFIG_KEYS.promptInsurerQuestions) as string | undefined) || undefined,
  }
}
...
export async function getReferenceDetails(orgId: string | null): Promise<{ labels: ReferenceLabelOverrides; hidden: ComputedRowKind[] }> {
  if (!orgId) return { labels: {}, hidden: [] }
  const svc = createServiceClient()
  ...
  return { labels: parseReferenceLabelOverrides(data?.value), hidden: parseReferenceHiddenKinds(data?.value) }
}
