-- Businesses pages group verifications by insured identity (legal name +
-- address). The address only existed inside coi_extracted, which is
-- publish-gated in my_verifications, so it gets its own column stamped by
-- extraction alongside insured_name. Ungated in the view: identity metadata,
-- not analysis output.

alter table verifications add column if not exists insured_address text;

-- Backfill from prior extractions; only rows still missing it, so re-runs and
-- admin edits are never clobbered.
update verifications
   set insured_address = coi_extracted->>'named_insured_address'
 where insured_address is null
   and coi_extracted->>'named_insured_address' is not null;

-- DROP + CREATE, not OR REPLACE, for the same reason as 0008/0027: the runner
